...
import (
	...
	gas "github.com/nxenv/rapidship/app/store"
	...
	"github.com/nxenv/rapidship/registry/app/manifest/ocischema"
	...
	"github.com/nxenv/rapidship/registry/app/store/database/util"
	"github.com/nxenv/rapidship/registry/gc"
	"github.com/nxenv/rapidship/registry/types"
	rapidshipstore "github.com/nxenv/rapidship/store"
	db "github.com/nxenv/rapidship/store/database"
...
func (l *manifestService) dbTagManifest(
	...
	if errors.Is(err, rapidshipstore.ErrResourceNotFound) {
...
func (l *manifestService) UpsertImage(
	...
	if err != nil && !errors.Is(err, rapidshipstore.ErrResourceNotFound) {
		return err
	} else if image != nil {
		return nil
	}
...
func (l *manifestService) validateManifestIndex(
	...
	for _, desc := range manifestList.Manifests {
		...
		if errors.Is(err, rapidshipstore.ErrResourceNotFound) && r.Type == artifact.RegistryTypeUPSTREAM {
			continue
		}
		if err != nil {
			return nil, nil, err
