...
import (
	...
	"github.com/nxenv/rapidship/app/gitspace/infrastructure"
	...
	gittypes "github.com/nxenv/rapidship/git/types"
	"github.com/nxenv/rapidship/infraprovider"
	"github.com/nxenv/rapidship/job"
	"github.com/nxenv/rapidship/lock"
	"github.com/nxenv/rapidship/pubsub"
	"github.com/nxenv/rapidship/store/database"
...
func LoadConfig() (*types.Config, error) {
	...
	if config.Git.Root == "" {
		...
		newPath := filepath.Join(homedir, rapidshipHomeDir)
...
func ProvideBlobStoreConfig(config *types.Config) (blob.Config, error) {
	...
	if config.BlobStore.Provider == blob.ProviderFileSystem && config.BlobStore.Bucket == "" {
		...
		config.BlobStore.Bucket = filepath.Join(homedir, rapidshipHomeDir, blobDir)
	}
	return blob.Config{
...
// ProvideLockConfig generates the `lock` package config from the Nxenv config.
...
func ProvideDockerConfig(config *types.Config) (*infraprovider.DockerConfig, error) {
	if config.Docker.MachineHostName == "" {
		rapidshipBaseURL, err := url.Parse(config.URL.Base)
		if err != nil {
			return nil, fmt.Errorf("unable to parse Nxenv base URL %s: %w", rapidshipBaseURL, err)
		}
		config.Docker.MachineHostName = rapidshipBaseURL.Hostname()
	}
