...
// Export creates a new empty repository in nxenv code and does git push to it.
func (c *Controller) Export(ctx context.Context, session *auth.Session, spaceRef string, in *ExportInput) error {
	...
	providerInfo := &exporter.NxenvCodeInfo{
...
func (c *Controller) sanitizeExportInput(in *ExportInput) error {
	...
	if in.Token == "" {
		return usererror.BadRequest("token for nxenv code must be provided")
	}
