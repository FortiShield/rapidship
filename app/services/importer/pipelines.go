...
func convertPipelineFiles(ctx context.Context,
	...
	const (
		nxenvPipelineName     = "pipeline"
		nxenvPipelineNameOnly = "default-" + nxenvPipelineName
		nxenvPipelineDir      = ".nxenv"
		nxenvPipelineFileOnly = nxenvPipelineDir + "/pipeline.yaml"
	)
	...
	for _, file := range files {
		...
		if len(files) == 1 {
			pipelineName = nxenvPipelineNameOnly
			pipelinePath = nxenvPipelineFileOnly
		} else {
			base := path.Base(file.OriginalPath)
			base = strings.TrimSuffix(base, path.Ext(base))
			pipelineName = nxenvPipelineName + "-" + base
			pipelinePath = nxenvPipelineDir + "/" + base + ".yaml"
		}
