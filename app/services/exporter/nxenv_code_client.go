...
// newClient creates a new nxenv Client for interacting with the platforms APIs.
...
func newNxenvCodeClient(
	baseURL string,
	accountID string,
	orgID string,
	projectID string,
	token string,
) (*nxenvCodeClient, error) {
	client, err := newClient(baseURL, accountID, orgID, projectID, token)
	if err != nil {
		return nil, err
	}
	return &nxenvCodeClient{
		client: client,
	}, nil
}

func (c *nxenvCodeClient) CreateRepo(ctx context.Context, input repo.CreateInput) (*types.Repository, error) {
...
func (c *nxenvCodeClient) DeleteRepo(ctx context.Context, repoIdentifier string) error {
