...
import (
	...
	rapidshipurl "github.com/nxenv/rapidship/app/url"
...
const (
	...
	nxenvCodeAPIURLRaw = "https://app.nxenv.io/gateway/code/api"
)
...
type Repository struct {
	urlProvider  rapidshipurl.Provider
	git          git.Interface
...
type Input struct {
	...
	NxenvCodeInfo NxenvCodeInfo `json:"nxenv_code_info"`
}

type NxenvCodeInfo struct {
...
func (r *Repository) Handle(ctx context.Context, data string, _ job.ProgressReporter) (string, error) {
	...
	nxenvCodeInfo := input.NxenvCodeInfo
	client, err := newNxenvCodeClient(
		nxenvCodeAPIURLRaw,
		nxenvCodeInfo.AccountID,
		nxenvCodeInfo.OrgIdentifier,
		nxenvCodeInfo.ProjectIdentifier,
		nxenvCodeInfo.Token,
	)
	if err != nil {
		return "", err
	...
	urlWithToken, err := modifyURL(remoteRepo.GitURL, nxenvCodeInfo.Token)
	if err != nil {
		return "", err
	}
	...
	if err != nil && !strings.Contains(err.Error(), "empty") {
		...
		if errDelete != nil {
			log.Ctx(ctx).Err(errDelete).Msgf("failed to delete repo '%s' on nxenv", remoteRepo.Identifier)
		}
		r.sseStreamer.Publish(ctx, repository.ParentID, enum.SSETypeRepositoryExportCompleted, repository)
		return "", err
	}

	log.Ctx(ctx).Info().Msgf("completed exporting repository '%s' to nxenv", repository.Identifier)
