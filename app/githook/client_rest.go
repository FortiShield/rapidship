...
// RestClientFactory creates clients that make rest api calls to Nxenv to execute githooks.
...
// RestClient is the hook.Client used to call the githooks api of Nxenv api server.
...
// PreReceive calls the pre-receive githook api of the Nxenv api server.
...
// Update calls the update githook api of the Nxenv api server.
...
// PostReceive calls the post-receive githook api of the Nxenv api server.
...
func (c *RestClient) githook(ctx context.Context, githookType string, payload interface{}) (hook.Output, error) {
	...
	req.Header.Add(request.HeaderUserAgent, fmt.Sprintf("Rapidship/%s", version.Version))
