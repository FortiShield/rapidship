...
import (
	...
	rapidship_store "github.com/nxenv/rapidship/store"
...
func (s *RepoStore) Update(ctx context.Context, repo *types.Repository) error {
	...
	if count == 0 {
		return rapidship_store.ErrVersionConflict
	}
...
func (s *RepoStore) UpdateSize(ctx context.Context, id int64, sizeInKiB int64) error {
	...
	if count == 0 {
		return fmt.Errorf("repo %d size not updated: %w", id, rapidship_store.ErrResourceNotFound)
	}
...
func (s *RepoStore) UpdateOptLock(
	...
	return s.updateOptLock(
		...
		func(r *types.Repository) error {
			if repo.Deleted != nil {
				return rapidship_store.ErrResourceNotFound
			}
			return mutateFn(r)
		},
	)
...
func (s *RepoStore) updateOptLock(
	...
	for {
		...
		if !errors.Is(err, rapidship_store.ErrVersionConflict) {
			return nil, err
		}
...
func (s *RepoStore) getRepoPath(ctx context.Context, parentID int64, repoIdentifier string) (string, error) {
	...
	if errors.Is(err, rapidship_store.ErrResourceNotFound) {
