...
import type { TypesGitspaceConfig } from 'cde-rapidship/services'
...
export const useLisitngApi = ({
  ...
  const rapidship = useGet<TypesGitspaceConfig[]>({
  ...
  useEffect(() => {
    if (standalone) {
      rapidship.refetch({ queryParams: { ...pageBrowser, page, limit } })
    } else {
      const queryParams = {
        ...pageBrowser,
        page,
