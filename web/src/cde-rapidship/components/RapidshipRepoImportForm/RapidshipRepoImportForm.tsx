...
import { Button, ButtonVariation, Container, ExpandingSearchInput, Layout, Text } from '@nxenvio/uicore'
import { Menu, MenuItem } from '@blueprintjs/core'
import { Color } from '@nxenvio/design-system'
import { Icon } from '@nxenvio/icons'
...
import noRepo from 'cde-rapidship/assests/noRepo.svg?url'
import { RepoCreationType } from 'utils/GitUtils'
import rapidshipRepoLogo from 'cde-rapidship/assests/rapidship.svg?url'
import { EnumGitspaceCodeRepoType } from 'cde-rapidship/constants'
import { useQueryParams } from 'hooks/useQueryParams'
import type { RepoQueryParams } from 'cde-rapidship/pages/GitspaceCreate/CDECreateGitspace'
import { GitspaceSelect } from '../GitspaceSelect/GitspaceSelect'
import css from './RapidshipRepoImportForm.module.scss'
  ...
  return (
    <Layout.Horizontal spacing={'medium'} flex={{ justifyContent: 'flex-start', alignItems: 'center' }}>
      {isCDE ? (
        ...
        <img src={rapidshipRepoLogo} height={24} width={24} />
      )}
      {repoName ? (
...
export const RapidshipRepoImportForm = ({ isCDE }: { isCDE?: boolean }) => {
  const { getString } = useStrings()
                  ...
                  <MenuItem
                    ...
                    text={
                      <Layout.Horizontal spacing="medium">
                        <img src={rapidshipRepoLogo} height={16} width={16} />
                        <Text>{repo.path}</Text>
                      </Layout.Horizontal>
                    }
                    ...
                    onClick={() => {
                      ...
                      formik.setValues((prvValues: any) => {
                        const codeRepoType = isCDE ? {} : { code_repo_type: EnumGitspaceCodeRepoType.RAPIDSIP }
                        return {
                          ...prvValues,
