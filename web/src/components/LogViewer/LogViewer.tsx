...
import { Button, ButtonSize, ButtonVariation, Container, FlexExpander, Layout, Text, Utils } from '@nxenvio/uicore'
...
export type EnumCheckPayloadKindExtended = EnumCheckPayloadKind | 'nxenv_stage'
...
const LogTerminal: React.FC<LogViewerProps> = ({
  ...
  return (
    <>
      {steps && (selectedItemData?.payload?.kind as EnumCheckPayloadKindExtended) === 'nxenv_stage' ? (
        renderedSteps
      ) : (
...
export const LogStageContainer: React.FC<LogStageContainerProps> = ({
  ...
  return <Container key={`nxenvlog_${value}`} ref={localRef} className={cx(css.main, css.stepLogContainer)} />
}
