# Nxenv
Nxenv Open Source is an open source development platform packed with the power of code hosting, automated DevOps pipelines, hosted development environments (Gitspaces), and artifact registries.

## Overview
Nxenv Open source is an open source development platform packed with the power of code hosting, automated DevOps pipelines, Gitspaces, and artifact registries.


## Running Nxenv locally
> The latest publicly released docker image can be found on [nxenv/nxenv](https://hub.docker.com/r/nxenv/nxenv).

To install Nxenv yourself, simply run the command below. Once the container is up, you can visit http://localhost:3000 in your browser.
...
docker run -d \
  ...
  nxenv/nxenv
```
> The Nxenv image uses a volume to store the database and repositories. It is highly recommended to use a bind mount or named volume as otherwise all data will be lost once the container is stopped.

See [developer.nxenv.io](https://developer.nxenv.io/docs/open-source) to learn how to get the most out of Nxenv.

## Where is Drone?

Nxenv Open Source represents a massive investment in the next generation of Drone. Where Drone focused solely on continuous integration, Nxenv adds source code hosting, developer environments (gitspaces), and artifact registries; providing teams with an end-to-end, open source DevOps platform.

The goal is for Nxenv to eventually be at full parity with Drone in terms of pipeline capabilities, allowing users to seamlessly migrate from Drone to Nxenv.

But, we expect this to take some time, which is why we took a snapshot of Drone as a feature branch [drone](https://github.com/nxenv/nxenv/tree/drone) ([README](https://github.com/nxenv/nxenv/blob/drone/.github/readme.md)) so it can continue development.

As for Nxenv, the development is taking place on the [main](https://github.com/nxenv/nxenv/tree/main) branch.

For more information on Nxenv, please visit [developer.nxenv.io](https://developer.nxenv.io/).

For more information on Drone, please visit [drone.io](https://www.drone.io/).

## Nxenv Open Source Development
### Pre-Requisites
...
After that, you can build the Nxenv binary:
...
```bash
./rapidship server .local.env
```

### Auto-Generate Nxenv API Client used by UI using Swagger
...
- Regenerate swagger with latest Nxenv binary `./rapidship swagger > web/src/services/code/swagger.yaml`
...
For testing, it's simplest to just use the cli to create a token (this requires Nxenv server to run):
```bash
# LOGIN (user: admin, pw: changeit)
$ ./rapidship login

# GENERATE PAT (1 YEAR VALIDITY)
$ ./rapidship user pat "my-pat-uid" 2592000
```
...
Refer to [CONTRIBUTING.md](https://github.com/nxenv/nxenv/blob/main/CONTRIBUTING.md)

## License

Apache License 2.0, see [LICENSE](https://github.com/nxenv/nxenv/blob/main/LICENSE).
