...
dep: $(deps) ## Install the deps required to generate code and build Nxenv
...
###############################################################################
#
# Nxenv Build and testing rules
#
###############################################################################

build: generate ## Build the all-in-one Nxenv binary
	@echo "Building Nxenv Server"
	go build -o ./rapidship ./cmd/rapidship
...
run: ar-clean build
	./rapidship server .local.env || true

ar-conformance-test: ar-clean build
	./rapidship server .local.env > logfile.log 2>&1 & echo $$! > server.PID
	@sleep 10
...
format: tools # Format go code and error if any changes are made
	@echo "Formatting ..."
	@goimports -w .
	@gci write --skip-generated --custom-order -s standard -s "prefix(github.com/nxenv/rapidship)" -s default -s blank -s dot .
...
wire: cmd/rapidship/wire_gen.go

force-wire: ## Force wire code generation
	@sh ./scripts/wire/rapidship.sh

cmd/rapidship/wire_gen.go: cmd/rapidship/wire.go
	@sh ./scripts/wire/rapidship.sh
