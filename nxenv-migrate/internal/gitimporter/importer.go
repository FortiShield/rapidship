...
import (
	...
	"github.com/nxenv/rapidship/nxenv-migrate/internal/types/enum"
...
type Importer struct {
	...
	NxenvSpace    string
	NxenvRepo     string // optional for single repo import
	NxenvToken    string
	Endpoint        string
...
func NewImporter(
	...
	client := nxenv.New(spaceParts[0], token, nxenv.WithAddress(baseURL), nxenv.WithTracing(trace))

	if gitness {
		client = nxenv.NewGitness(token, baseURL, nxenv.WithTracing(trace))
	}
...
func (m *Importer) Import(ctx context.Context) error {
	...
	folders, err := getRepoBaseFolders(unzipLocation, m.NxenvRepo)
	if err != nil {
	...
	for _, f := range folders {
		...
		repoRef := util.JoinPaths(m.NxenvSpace, repository.Name)
		...
		if err := m.createRepoAndDoPush(ctx, f, &repository); err != nil {
			...
			if !errors.Is(err, nxenv.ErrDuplicate) {
		...
		_, err = m.Nxenv.UpdateRepositoryState(
			repoRef,
			&nxenv.UpdateRepositoryStateInput{State: enum.RepoStateMigrateDataImport},
		)
		if err != nil {
		...
		_, err = m.Nxenv.UpdateRepositoryState(
			repoRef,
			&nxenv.UpdateRepositoryStateInput{State: enum.RepoStateActive},
		)
		if err != nil {
...
func (m *Importer) createRepoAndDoPush(ctx context.Context, repoFolder string, repo *types.Repository) error {
	hRepo, err := m.CreateRepo(repo, m.NxenvSpace, m.Tracer)
	if err != nil {
	...
	repoRef := util.JoinPaths(m.NxenvSpace, repo.Name)
...
func (m *Importer) importRepoMetaData(_ context.Context, repoRef, repoFolder string) error {
	if !m.flags.NoLabel {
		...
		if err := m.ImportLabels(repoRef, repoFolder); err != nil && !errors.Is(err, nxenv.ErrNotFound) {
...
func (m *Importer) cleanup(repoRef string) {
	...
	err := m.Nxenv.DeleteRepository(repoRef)
	if err != nil {
...
func notRecoverableError(err error) bool {
	if errors.Is(err, nxenv.ErrForbidden) ||
		errors.Is(err, nxenv.ErrUnauthorized) ||
		errors.Is(err, nxenv.ErrNotFound) ||
		errors.Is(err, nxenv.ErrInvalidRef) {
		return true
	}
