...
func (m *Importer) Import(ctx context.Context, data *types.Org) error {
	...
	for _, secret := range data.Secrets {
		if _, err = m.Nxenv.FindSecretOrg(org.ID, secret.Name); err != nil {
			s := util.CreateSecretOrg(org.ID, secret.Name, secret.Value)
			// save the secret to the organization
			if err := m.Nxenv.CreateSecretOrg(s); err != nil {
				return err
			}
		}
	...
	if repoConn == "" {
		...
		foundConnector, err := m.Nxenv.FindConnectorOrg(org.ID, m.ScmType)
		if err != nil || foundConnector == nil {
			m.Tracer.Start("create connector %s", m.ScmType)
			var conn *nxenv.Connector
			switch m.ScmType {
			case "gitlab":
	...
	if dockerConn == "" {
		...
		existingConnector, err := m.Nxenv.FindConnectorOrg(org.ID, dockerConnectorName)
	...
	// convert each drone repo to a nxenv project.
	...
	for _, srcProject := range data.Projects {
		...
		// convert the circle project to a nxenv project
		...
		if err := nxenv.WaitNxenvSecretManager(
			m.Nxenv, m.NxenvOrg, projectSlug); err != nil {
			return err
		}
		...
		for _, srcEnv := range srcProject.Secrets {
			...
			// secret, stored in the nxenv secret manager.
			secret := util.CreateSecret(org.ID, projectSlug, slug.Create(srcEnv.Name), srcEnv.Desc, srcEnv.Value)
			// save the secret to nxenv.
			if err := m.Nxenv.CreateSecret(secret); err != nil {
		...
		if m.Downgrade {
			d := downgrader.New(
				...
				downgrader.WithOrganization(m.NxenvOrg),
		...
		//create the nxenv pipeline with an inline yaml
		err = m.Nxenv.CreatePipeline(org.ID, projectSlug, srcProject.Yaml)
		if err != nil {
