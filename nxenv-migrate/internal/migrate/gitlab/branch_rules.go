...
func (e *Export) ListBranchRules(
	...
	MRRule, err := e.GetMergeRequestRules(ctx, repoSlug)
	if err != nil {
		e.tracer.LogError("failed to get project info for merge request settings: %w", err)
		e.tracer.Stop(common.MsgFailedExportBranchRules, repoSlug)
		return nil, fmt.Errorf(common.MsgFailedExportBranchRules, repoSlug)
	}

	allRules = append(allRules, MRRule)
	for {
