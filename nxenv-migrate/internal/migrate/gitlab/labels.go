...
func (e *Export) ListLabels(
	...
	for {
		labels, res, err := e.ListRepoLabels(ctx, repoSlug, types.ListOptions{
			Page: opts.Page,
			Size: 20,
		})
		if err != nil {
		...
		// in Gitlab the combination of label's "name::value" is unique.
		for _, label := range labels {
			allLabels[label.Name+"::"+label.Value] = label
		}
