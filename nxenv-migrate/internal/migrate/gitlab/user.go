...
func (e *Export) FindEmailByUsername(ctx context.Context, username string) (string, error) {
	...
	if errors.Is(err, nxenv.ErrNotFound) {
...
func (e *Export) FindEmailByUserID(ctx context.Context, id int) (string, error) {
	...
	if errors.Is(err, nxenv.ErrNotFound) || userData == nil {
		return "", fmt.Errorf("couldn't find the user with id %d: %w", id, nxenv.ErrNotFound)
	}
