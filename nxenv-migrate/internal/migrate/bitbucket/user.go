...
func (e *Export) FindEmailByAccountID(ctx context.Context, accID string) (string, error) {
	...
	if errors.Is(err, nxenv.ErrForbidden) {
		return "",
			fmt.Errorf("token must have 'user' scope to fetch workspace members emails: %w", nxenv.ErrForbidden)
	}

	if err != nil && !errors.Is(err, nxenv.ErrNotFound) {
		return "", err
	}

	if errors.Is(err, nxenv.ErrNotFound) || u == nil {
		return "", fmt.Errorf("couldn't find the user with account id %s: %w", accID, nxenv.ErrNotFound)
	}
