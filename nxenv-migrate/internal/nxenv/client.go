...
// Package nxenv provides a Nxenv http client.
package nxenv
...
// Client is used to communicate with the Nxenv server.
type Client interface {
	...
	// CheckUsers provides all email id to nxenv code of users which needs to be checked for existence.
	CheckUsers(in *types.CheckUsersInput) (*types.CheckUsersOutput, error)
}

// WaitNxenvSecretManager blocks until the nxenv
// secret manager is created for the project.
func WaitNxenvSecretManager(client Client, org, project string) error {
	for i := 0; ; i++ {
		if _, err := client.FindConnector(org, project, "nxenvSecretManager"); err == nil {
			return nil
		} else if i == 30 {
			return err
		} else {
...
// WaitNxenvSecretManagerOrg blocks until the nxenv
// secret manager is created for the organization.
func WaitNxenvSecretManagerOrg(client Client, org string) error {
	for i := 0; ; i++ {
		if _, err := client.FindConnectorOrg(org, "nxenvSecretManager"); err == nil {
			return nil
		} else if i == 30 {
			return err
		} else {
