...
func Register(app *kingpin.Application) {
	...
	cmd.Flag("account", "nxenv account ID").
		StringVar(&c.account)

	cmd.Flag("endpoint", "nxenv endpoint").
		Default("https://app.nxenv.io/gateway").
	...
	cmd.Flag("provider-source", "nxenv terraform provider source").
		Default("nxenv/nxenv").
		StringVar(&c.providerSource)

	cmd.Flag("provider-version", "nxenv terraform provider version").
		Default("0.19.1").
