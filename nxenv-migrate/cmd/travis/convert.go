...
func (c *convertCommand) run(ctx *kingpin.ParseContext) error {
	...
	// format to the nxenv yaml format.
	...
	// downgrade from the v1 nxenv yaml format
	// to the v0 nxenv yaml format.
	if c.downgrade {
...
func registerConvert(app *kingpin.CmdClause) {
	...
	cmd := app.Command("convert", "convert a travis yaml").
		Action(c.run)

	cmd.Arg("input", "path to travis yaml directory or file").
		Default(".travis.yml").
		StringVar(&c.input)
	...
	cmd.Flag("org", "nxenv organization").
		Default("default").
		StringVar(&c.org)

	cmd.Flag("project", "nxenv project").
		Default("default").
		StringVar(&c.proj)

	cmd.Flag("pipeline", "nxenv pipeline name").
		Default("default").
		StringVar(&c.name)
