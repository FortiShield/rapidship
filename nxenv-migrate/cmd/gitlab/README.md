...
- Any attachment
- LFS objects
- Webhooks: Some webhook events are not supported. You can check supported triggers [here](https://apidocs.nxenv.io/tag/webhook#operation/createWebhook)
...
## Webhooks
When they are exported, supported Gitlab webhooks are stored in zip file, which later during import to nxenv code are mapped according to:

### Branch Protection and Webhooks
When they are exported, supported Gitlab branch protection rules and webhooks are stored in zip file, which later during import to nxenv code are mapped according to:

### Webhooks
| Gitlab events | Nxenv Code events
|---|---|
...
### Branch protection rules 
| Gitlab branch protection rule | Nxenv Code rule
|---|---|
...
| Gitlab merge method | Nxenv Code merge strategies (allowed)
|---|---|
...
Note Gitlab branch protection rules follow the "most permissive" rule if multiple rules can apply to a single branch. Nxenv Code follows the "Apply all" rule.

Group-level merge request approvals need to be set manually after the migration. Refer to `Require a minimum number of reviewers` rule in the [Docs](https://developer.nxenv.io/docs/code-repository/config-repos/rules/#available-rules).
