## Support
Import repositories to Nxenv Code Repository with metadata including:
...
```sh
./migrator git-import <zip-folder-path>  --space <target acc/org/project> --endpoint <nxenv-url> --token <token>
```
...
```sh
export nxenv_TOKEN={nxenv-pat}
./migrator git-import ./nxenv/nxenv.zip  --space "acc/MyOrg/Myproject" --endpoint "https://app.nxenv.io/"  --skip-users  --skip-pr --skip-webhook --skip-rule --file-size-limit 102000000
```
...
| Error | Resolution |
|---|---|
| Resource not found | Target space must exist on Nxenv. You can import repositories into account, organization or projects. |
...
### Users
When exporting repositories, we collect information about users who have interacted with them. This data is saved in a `users.json` file within the exported zip. During the import process, if a user exists on the Nxenv platform with the same email, their activities will be preserved. However, if the user is missing, you have two options: using `--skip-users` which skips mapping their activities to the migrator (determined by the token you provided) or manually create the user on the target platform first before the import.
