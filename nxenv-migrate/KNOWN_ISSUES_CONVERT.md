...
NOTE: In this document, "v0 yaml" refers to current Nxenv CI yaml, "v1 yaml" 
refers to the new "simplified" Nxenv CI yaml.

`nxenv-migrate` supports converting to v0 yaml with the `--downgrade` flag,
for example:

```
nxenv-migrate github convert --downgrade example.yml
```

To see all supported conversion flags for a provider, pass the `--help` flag:

```
nxenv-migrate github convert --help
```
...
Kubernetes-based Nxenv CI pipeline steps run with default resource limits.

**Problem**

`nxenv-migrate` does not set resource limits during pipeline conversion.

**Solution**

Manually set the proper [resource limits](https://developer.nxenv.io/docs/continuous-integration/use-ci/run-ci-scripts/run-step-settings/#set-container-resources
) for your step(s) that run on Kubernetes infrastructure.

### JEXL syntax

Conversion will not always choose the correct [JEXL condition syntax](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/triggers-reference/#conditions-settings).
...
See [Webhook Triggers Reference](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/triggers-reference/)
...
Nxenv CI [webhook triggers](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/triggers-reference/) are managed separately from the pipeline yaml.

Nxenv CI also supports conditions at the [stage and step level](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/step-skip-condition-settings/) which are managed in the yaml.
...
Push and pull request webhook triggers in Nxenv CI can be updated to use this payload condition to maintain the same behavior:
...
Drone variables must be converted to the equivalent [Nxenv variable expression](https://developer.nxenv.io/docs/platform/variables-and-expressions/extracting-characters-from-nxenv-variable-expressions/).
...
Examine your converted pipeline yaml for any `DRONE_` variables that were not converted. Replace these variables with the equivalent Nxenv JEXL expression.

**Problem**

In Drone, substituted variables with no value will be empty. In Nxenv CI expressions, unset values will return a `null` string. **This may break logic that relies on an empty string for unset values**.
...
Drone provides partial emulation for bash string operations, these must convert to an equivalent [Nxenv variable expression](https://developer.nxenv.io/docs/platform/variables-and-expressions/extracting-characters-from-nxenv-variable-expressions/).
...
Examine your converted pipeline yaml for any `DRONE_` variables with string operations that were not converted. Replace these with the equivalent Nxenv JEXL expression.
...
Nxenv CI does not have equivalent support for dependencies at the step level.
...
Nxenv CI does not support Docker config.json files in connectors.
...
Nxenv CI does not currently support timeouts at the stage level.
