Level of conversion support for GitLab [job keywords](https://docs.gitlab.com/ee/ci/yaml/#job-keywords) to Nxenv CI YAML.
...
| 🔴 | Unsupported | Conversion either requires investigation, or the feature is not yet supported by Nxenv CI |
...
Notes:
- This is supported at the trigger level with the **Auto-abort Previous Execution** setting, see [Trigger pipelines using Git events](https://developer.nxenv.io/docs/platform/triggers/triggering-pipelines/)
...
Notes:
- There is likely an equivalent JEXL expression, see [Webhook triggers reference](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/triggers-reference/)
...
Notes:
- GitLab sets `CI_NODE_INDEX` and `CI_NODE_TOTAL` variables, Nxenv CI sets `<+strategy.iteration>` and `<+strategy.iterations>`
- `CI_NODE_INDEX` starts at `1` in GitLab, `<+strategy.iteration>` starts at `0` in Nxenv CI
- See [Speed up CI test pipelines using parallelism](https://developer.nxenv.io/docs/platform/pipelines/speed-up-ci-test-pipelines-using-parallelism/)
...
Notes:
- Matrix values in Nxenv CI do not become environment variables by default, they must be added to `envVariables`
- See [Looping strategies overview](https://developer.nxenv.io/docs/platform/pipelines/looping-strategies-matrix-repeat-and-parallelism/#matrix)
...
Notes:
- See [Define a failure strategy on stages and steps](https://developer.nxenv.io/docs/platform/pipelines/define-a-failure-strategy-on-stages-and-steps/#add-a-step-failure-strategy)
...
Notes:
- Secrets are converted to environment variable placeholders, secrets must still be created in the Nxenv project
...
Notes:
- Vault is supported by Nxenv CI, secrets must be added manually
- See [Add a HashiCorp Vault secret manager](https://developer.nxenv.io/docs/platform/secrets/secrets-management/add-hashicorp-vault/)

### 🟠 [`secrets:azure_key_vault`](https://docs.gitlab.com/ee/ci/yaml/#secretsazure_key_vault)

Notes:
- Azure Key Vault is supported by Nxenv CI, secrets must be added manually
- See [Add an Azure Key Vault secret manager](https://developer.nxenv.io/docs/platform/secrets/secrets-management/azure-key-vault/)
...
Notes:
- Services can be converted to a [background step](https://developer.nxenv.io/docs/continuous-integration/use-ci/manage-dependencies/background-step-settings/)
- `alias` is not supported
- See [Background step settings](https://developer.nxenv.io/docs/continuous-integration/use-ci/manage-dependencies/background-step-settings/#port-bindings) for communicating with background steps by Id
            ...
            steps:
              - parallel:
                  - step:
                      ...
                      spec:
                        connectorRef: account.nxenvImage
                        image: postgres:11.6
                        shell: Sh
                        imagePullPolicy: IfNotPresent
                  - step:
...
Notes:
- Can be manually converted to [runtime inputs](https://developer.nxenv.io/docs/platform/references/runtime-inputs/#using-allowed-values-default-values-and-multiple-selection-in-runtime-inputs)
...
Notes:
- See [Stage and step conditional execution settings](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/step-skip-condition-settings/)

### 🟠 `when:on_failure`

Notes:
- Add a [step condition](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/step-skip-condition-settings/#step-conditions) to the converted step
...
Notes:
- Add a [step condition](https://developer.nxenv.io/docs/platform/pipelines/w_pipeline-steps-reference/step-skip-condition-settings/#step-conditions) to the converted step that references [`pipeline.triggerType`](https://developer.nxenv.io/docs/platform/variables-and-expressions/nxenv-variables/#pipelinetriggertype)
